...
namespace SKDVS.Utils.ErrorHandler
{
	...
	public class SimpleMail
	{
		...
		public class SMTPClient
		{
            ...
            private string _strDefaultDomain = "smurfitkappa.com";
			private string _strServer = "relay.group.wan";
			...
			private bool CommandInternal(TcpClient tcp, string strCommand, string strExpectedResponse = "250")
			{
				...
				while ((string.IsNullOrEmpty(_strLastResponse)) & (intResponseTime <= _intResponseTimeMax)) {
                    ...
                    //System.Threading.Thread.CurrentThread.Sleep(_intResponseTimeExpected); ???
			...
			public bool SendMail(SMTPMailMessage mail)
			{
				...
				try {
					...
					if (_intRetries <= _intMaxRetries) {
                        ...
                        System.Threading.Thread.Sleep(intRetryInterval);
						SendMail(mail);
					} else {
						throw;
					}
